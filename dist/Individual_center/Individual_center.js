...
function modify_password(){
    ...
    window.alert("若需要返回原界面，直接点解修改密码按键即可！");
}
...
function send_message_button() {
    ...
    console.log(data);
    if(tag_input == '')  window.alert();
    if(path_input != '' && tag_input != ''){
        var xhttp = new XMLHttpRequest();
        xhttp.onreadystatechange = function () {
            if(this.readyState == 4 && this.status == 200) {
                flush_pdf_table();
        ...
        var send_message = "account=" + account_receive + "&name=" + name_input
            + "&path=" + path_input + "&tag=" + tag_input + "&data=" + data + "&lang=" + lang_input;
        xhttp.setRequestHeader("Content-type","application/x-www-form-urlencoded");
        xhttp.send(send_message);
    }
}
        ...
        if(this.readyState == 4 && this.status == 200) {
            if(str == '') flush_pdf_table();
                ...
                if(xmlDoc.getElementsByTagName("name").length == 0){
                    ...
                    document.getElementById('pdf_show_table').remove();
                    console.log(xmlDoc);
                    console.log(xmlDoc.getElementsByTagName("name").length);
